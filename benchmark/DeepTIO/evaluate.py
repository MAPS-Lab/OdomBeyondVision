...
parser.add_argument('--dim', help='evaluate on 2D or 3D spaces', default='3')
...
# models = ['deeptio_handheld']
models = ['tio3_gap2']

data_dir = join(cfg['eval']['h5_path'], 'test')
...
for seq_idx in seqs:
    ...
    for model in models:
        model_dir = join(cfg['eval']['model_dir'], model)
        try:
        ...
        for epoch in epochs:
            ...
            gt_name = 'gt_seq' + seq_idx
            gt_path = join('./results', gt_name)
            time_path = join('./results', time_name)
            q_gt_path = join('./results', 'q_' + gt_name)
            cmd = 'python -W ignore ' + 'eval/convert_pose_to_quat.py' + ' ' + '--pose ' + gt_path + ' ' + '--timestamp ' + time_path \
                  + ' ' + '--save_path ' + q_gt_path + ' ' + '--is_gt ' + '1'
            os.system(cmd)
            ...
            if args.dim == '2':
                cmd = 'python -W ignore ' + 'eval/evaluate_std_rpe_2D.py ' + q_gt_path + ' ' + q_prediction_path \
                      + ' ' + '--max_difference ' + max_diff + ' --err_comp ' + rte_csv_path + \
                      ' --model ' + pose_name + ' --verbose'
            elif args.dim == '3':
                cmd = 'python -W ignore ' + 'eval/evaluate_std_rpe.py ' + q_gt_path + ' ' + q_prediction_path \
                      + ' ' + '--max_difference ' + max_diff + ' --err_comp ' + rte_csv_path + \
                      ' --model ' + pose_name + ' --verbose'
            print(cmd)
            os.system(cmd)
            ...
            if args.dim == '2':
                cmd = 'python -W ignore ' + 'eval/evaluate_std_ate_2D.py ' + q_gt_path + ' ' + q_prediction_path + ' ' + '--max_difference ' + \
                      max_diff + ' ' + ' --err_comp ' + ate_csv_path + ' --model ' + pose_name + ' --verbose'
            elif args.dim == '3':
                cmd = 'python -W ignore ' + 'eval/evaluate_std_ate.py ' + q_gt_path + ' ' + q_prediction_path + ' ' + '--max_difference ' + \
                      max_diff + ' ' + ' --err_comp ' + ate_csv_path + ' --model ' + pose_name + ' --verbose'
            os.system(cmd)
