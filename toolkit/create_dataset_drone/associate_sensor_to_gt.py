...
DESCRIPTION = """This script receives a path of directory to master images and match
                with the (pseudo) ground truth csv according to their timestamp."""

parser = argparse.ArgumentParser(description=DESCRIPTION)
parser.add_argument('--sensor_dir', required=True, help='''Specify the master
                    folder (e.g. mmwave) you want to associate with lidar timestamp.''')
parser.add_argument('--gt', required=True, help='''Specify the GT pose
                    file (.csv) you want to match with the master timestamps.''')
...
log_string_filename = os.path.join(args.save_dir, 'odom_' + os.path.splitext(os.path.basename(args.sensor_dir))[0] + '_logs.txt')

with open(log_string_filename, 'w') as fw:
    for pos, line in enumerate(log_strings):
        fw.write(line)

filename = os.path.join(args.save_dir, 'odom_' + os.path.splitext(os.path.basename(args.sensor_dir))[0] + '_ref.csv')
MAX_MINUTES = 600
img_counter = 0
