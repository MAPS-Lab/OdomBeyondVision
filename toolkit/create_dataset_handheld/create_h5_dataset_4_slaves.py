...
def main():
    ...
    for j in range(len(all_exps)):
        ...
        # save timestamp
        timestamp = [line[:-1].split(',')[4] for line in sampled_files] # timestamp idx for new ref is 5
        ...
        for line in sampled_files:
            gt_lines_float.append(np.array(
                [float(line[:-1].split(',')[4]),  # timestamp
                 float(line[:-1].split(',')[5]), float(line[:-1].split(',')[6]), float(line[:-1].split(',')[7]),
                 # translation
                 float(line[:-1].split(',')[8]), float(line[:-1].split(',')[9]),
                 float(line[:-1].split(',')[10]), float(line[:-1].split(',')[11])]))  # quaternion
        ...
        for k in range(0, len(sampled_files)):
            ...
            # Load IMU data
            imu_start = 13 # 14 - (5 - 4) # 4 slaves out of 5
            for l in range(30):
            ...
            print('Processing folder: ', all_exps[j], 'Total img idx ', str(total_img_counter),
                  ...
                  '. Slave ', slaves[3] ,' size: ', np.shape(train_slave_imu),)
        ...
        with open(data_summary_path, 'a+') as f:
            ...
            print('Slave 5 => ', slaves[3], ': ', np.shape(train_slave_imu_data_np), file=f)
        ...
        with h5py.File(file_save, 'w') as hf:
            ...
            hf.create_dataset(str(slaves[3] + '_data'), data=train_slave_imu_data_np)
