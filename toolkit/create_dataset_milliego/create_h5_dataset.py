...
def main():
    ...
    all_exps = cfg['dataset_creation_robot']['all_exp_files']
    ...
    for j in range(len(all_exps)):
        ...
        # save timestamp
        timestamp = [line[:-1].split(',')[5] for line in sampled_files]  # timestamp idx for new ref is 5
        ...
        for line in sampled_files:
            gt_lines_float.append(np.array(
                [float(line[:-1].split(',')[5]),  # timestamp
                 float(line[:-1].split(',')[6]), float(line[:-1].split(',')[7]), float(line[:-1].split(',')[8]),
                 # translation
                 float(line[:-1].split(',')[9]), float(line[:-1].split(',')[10]),
                 float(line[:-1].split(',')[11]), float(line[:-1].split(',')[12])]))  # quaternion
        ...
        for k in range(0, len(sampled_files)):
            ...
            if master == 'thermal' or master == 'mmwave_middle' or master == 'mmwave_all' or master == 'mmwave_all_clean' \
                    or master == 'lidar' or master == 'depth':
            ...
            for i in range(len(slaves) - 1):  # this 'i' is only used in this loop
                ...
                if slaves[i] == 'thermal' or slaves[i] == 'mmwave_middle' or slaves[i] == 'mmwave_all' or slaves[i] == 'mmwave_all_clean'\
                        or slaves[i] == 'lidar' or slaves[i] == 'depth':
                ...
                if i == 0:
                    ...
                    train_slave_3.append(slave_img)
                elif i == 3:
                    train_slave_4.append(slave_img)
            ...
            print('Processing folder: ', all_exps[j], 'Total img idx ', str(total_img_counter),
                  ...
                  '. Slave ', slaves[2], ' size: ', np.shape(train_slave_3),
                  '. Slave ', slaves[3], ' size: ', np.shape(train_slave_4),
                  '. Slave ', slaves[4], ' size: ', np.shape(train_slave_imu),)
        ...
        train_slave_3_data_np = np.array(train_slave_3)
        train_slave_4_data_np = np.array(train_slave_4)
        ...
        with open(data_summary_path, 'a+') as f:
            ...
            print('Slave 3 => ', slaves[2], ': ', np.shape(train_slave_3_data_np), file=f)
            print('Slave 4 => ', slaves[3], ': ', np.shape(train_slave_4_data_np), file=f)
            print('Slave 5 => ', slaves[4], ': ', np.shape(train_slave_imu_data_np), file=f)
        ...
        with h5py.File(file_save, 'w') as hf:
            ...
            hf.create_dataset(str(slaves[2] + '_data'), data=train_slave_3_data_np)
            hf.create_dataset(str(slaves[3] + '_data'), data=train_slave_4_data_np)
            hf.create_dataset(str(slaves[4] + '_data'), data=train_slave_imu_data_np)
