...
"""def transform(sequence_num, directory):
    path = configs.PREPROCESSED_DIR + directory + sequence_num
    if not os.path.exists(path):
        os.makedirs(path)
    if not os.path.exists(path + '/depth'):
        os.makedirs(path + '/depth')
    pose_gt_path = configs.ROOT_DIR + configs.KITTI_POSE_DIR + configs.POSE_OUTPUT_DIR + sequence_num + '.txt'
    copy2(pose_gt_path, path + '/preprocessed_gt.txt')

    lst = glob(configs.KITTI_DEPTH_DATA_DIR + directory + sequence_num + '/velodyne/' + '*.bin')
    v_fov, h_fov = (-24, 2.0), (-180, 180)
    for idx, bin_file in enumerate(tqdm(lst)):
        velo_points = load_from_bin(bin_file)
        pano_img = velo_points_2_pano(velo_points, v_res=0.42, h_res=0.35, v_fov=v_fov, h_fov=h_fov, depth=False)

        rst = np.asarray(pano_img, dtype=np.float32)
        np.savez_compressed(path + '/depth/' + str(idx), a=rst)
"""